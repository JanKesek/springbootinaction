package taco.springbootinaction.game;
...
public class RollingTheMonkey extends SimpleApplication implements ActionListener, PhysicsCollisionListener {
    ...
    public void simpleInitApp() {
        ...
        Material materialYellow = materialYellow();
        ...
        try {
            createIngredientsToCollect(space, materialYellow, rotation, translation, index, amount);
        } catch (IOException e) {
            e.printStackTrace();
        ...
        playerGeometry.setShadowMode(RenderQueue.ShadowMode.CastAndReceive);
    ...
    private Material materialGreen() {
        ...
        materialYellow.setBoolean("UseMaterialColors",true);
        materialYellow.setBoolean("HardwareShadows", true);
        materialYellow.setColor("Diffuse", new ColorRGBA(0.0431f, 0.7725f, 0.0078f, 1.0f));
        materialYellow.setColor("Specular", ColorRGBA.White);
        materialYellow.setFloat("Shininess", 64.0f);
        return materialYellow;
    }
    ...
    private Material materialRed() {
        ...
        materialYellow.setBoolean("UseMaterialColors",true);
        materialYellow.setBoolean("HardwareShadows", true);
        materialYellow.setColor("Diffuse", new ColorRGBA(0.9451f, 0.0078f, 0.0314f, 1.0f));
        materialYellow.setColor("Specular", ColorRGBA.White);
        materialYellow.setFloat("Shininess", 64.0f);
        return materialYellow;
    }
    ...
    private void createIngredientsToCollect(PhysicsSpace space, Material materialYellow, Quaternion rotation, Vector3f translation, int index, float amount) throws IOException, TimeoutException {
    ...
    public void simpleUpdate(float tpf) {
        ...
        channel.queueDeclare("tacocloud.ingredient", true, false, false, null);
        DeliverCallback deliverCallback = (consumerTag, delivery) -> {

            String message = new String(delivery.getBody(), "UTF-8")
                .replace("\"","").replace("[","").replace("]","");
            List<String> ingredients = new ArrayList<String>(Arrays.asList(message.split(",")));
            System.out.println(" [x] Received '" + message + "'");
            Quaternion rotation = new Quaternion();
            Vector3f translation = new Vector3f(0.0f, PICKUP_SIZE * 1.5f, -PICKUP_RADIUS);
            int index = 0;
            for(float angle = 0; angle < FastMath.TWO_PI; angle += ingredients.size()) {
                Geometry pickUp = new Geometry("pickUp" + (index++), new Box(PICKUP_SIZE,PICKUP_SIZE, PICKUP_SIZE));
                pickUp.setShadowMode(RenderQueue.ShadowMode.CastAndReceive);
                if(ingredients.get(index).equals("paprika")) {
                    pickUp.setMaterial(materialRed());
                }
                else if(ingredients.get(index).equals("cheese")) {
                    pickUp.setMaterial(materialYellow());
                }
                else {
                    pickUp.setMaterial(materialGreen());
                }
        ...
        try {
            channel.basicConsume("tacocloud.ingredient", true, deliverCallback, consumerTag -> {
                System.out.println("BLAD DOSTARCZENIA WIADOMOSCI");
            });
        } catch (IOException e) {
            e.printStackTrace();
        }
